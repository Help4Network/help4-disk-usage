...
mkdir -p "$TMP_DIR/cache/accounts" "$TMP_DIR/cache/locks" "$TMP_DIR/account-cache"
...
cpanel_html="$(env "${cpanel_env[@]}" "$ROOT_DIR/src/cpanel/index.live.pl")"
cpanel_nonce="$(sed -n 's/.*name="action_nonce" value="\([0-9a-f]*\)".*/\1/p' <<<"$cpanel_html" | head -n 1)"
grep -Eq '^[0-9a-f]{64}$' <<<"$cpanel_nonce"

cpanel_get="$(env "${cpanel_env[@]}" QUERY_STRING=refresh=1 "$ROOT_DIR/src/cpanel/index.live.pl")"
grep -q 'Request rejected' <<<"$cpanel_get"

cpanel_body="refresh=1&action_nonce=$cpanel_nonce"
cpanel_post="$(printf '%s' "$cpanel_body" | env "${cpanel_env[@]}" REQUEST_METHOD=POST CONTENT_LENGTH="${#cpanel_body}" "$ROOT_DIR/src/cpanel/index.live.pl")"
